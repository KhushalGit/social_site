...
<div class="row">
	...
	<div class="col-sm-8">
		<?php
			echo"
			<div>
				<div><img id='cover-img' class='img-rounded' src='coverimage/$user_cover' alt='cover'></div>
			...
			<div id='profile-img'>
				<img src='user/$user_image' alt='Profile' class='img-circle' width='180px' height='185px'>
				<form action='profile.php?u_id='$user_id' method='post' enctype='multipart/form-data'>
		...
		<?php
			...
			if(isset($_POST['submit'])){
				...
				if($u_cover==''){
					...
					move_uploaded_file($image_tmp, "coverimage/$u_cover.$random_number");
	...
	<?php
		if(isset($_POST['update'])){
				...
				if($u_image==''){
					...
					move_uploaded_file($image_tmp, "user/$u_image.$random_number");
...
<div class="row">
	...
	<div class="col-sm-2" style="background-color: #e6e6e6;text-align: center;left: 0.8%;border-radius: 5px;">
		...
		echo"
			...
			<p><strong>Class: </strong> $user_class</p><br>
	...
	<div class="col-sm-2">
	   <?php
		...
		$get_posts = "select * from posts where user_id='$u_id' ORDER by 1 DESC LIMIT 5";  //display latest 5 post only user
		...
		//now we will display the post for user
		...
		if($content=="No" && strlen($upload_image) >= 1)
		{
			...
			<div id='own_posts'>
				  ...
				  <a href='function/delete_post.php?post_id=$post_id' style='float:right;'><button class='btn btn-danger'>Delete</button></a><br>
			</div><br><br>
